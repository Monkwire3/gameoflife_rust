...
impl Grid {
    ...
    fn count_live_neighbors(&self, x_0: usize, y_0: usize) -> i32 {
        let mut live_neighbors = 0;

        for y_1 in (if y_0 > 0 { y_0 - 1 } else { y_0 })..(y_0 + 1) {
            for x_1 in (if x_0 > 0 { x_0 - 1 } else { x_0 })..(x_0 + 1) {
                if !(y_1 == y_0 && x_1 == x_0) {
    ...
    fn get_next_state(&mut self) -> [[bool; GRID_SIZE]; GRID_SIZE] {
        ...
        for y in 0..GRID_SIZE {
            for x in 0..GRID_SIZE {
                let live_neighboors = self.count_live_neighbors(y, x);
